...
# test script for testing the policy outside of a docker container
# DONT touch this code
if __name__ ==  '__main__':
    from isc_hanabi import *
    ...
    while not done:
        action0 = agent0.get_actions(obs)
        action1 = agent1.get_actions(obs)
        action = action0 if obs["current_player"] == 0 else action1
        print("action", action)

        obs, reward, done, info = env.step(action)

    print(obs["player_observations"][0]["fireworks"])
