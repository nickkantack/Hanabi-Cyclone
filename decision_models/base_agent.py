...
class BaseAgent(DecisionModel):
    ...
    def decide_move(self, observation, my_not_clues, other_player_not_clues, playable_probabilities, safely_discardable_probabilities, unneeded_probabilities,
        ...
        for index in range(len(legal_moves)):
            ...
            if action_reader.is_action_hint(candidate_move):
                ...
                if turns_left_after_this_one == 0:
                    legal_move_point_values[index] -= INSURMOUNTABLE_COST
                ...
                if move_type == action_reader.DISCARD:
                    ...
                    legal_move_point_values[index] += info_token_contribution
                    move_score_description += "{:.2f} for discarding when we have {} info tokens, ".format(info_token_contribution, info_tokens)
                
                    discard_contribution = self.value_good_discard * safely_discardable_probabilities[card_index]
                    legal_move_point_values[index] += discard_contribution
                    move_score_description += "{:.2f} for discardability, ".format(discard_contribution)
                    
                    force_safe_discard_contribution = -self.force_safe_discard_factor * unneeded_probabilities[card_index]
                    legal_move_point_values[index] += force_safe_discard_contribution
                    move_score_description += "{:.2f} since unneeded prob is {:.2f}, ".format(force_safe_discard_contribution, unneeded_probabilities[card_index])
                    ...
                    if turns_left_after_this_one is not None and life_tokens > turns_left_after_this_one:
                        legal_move_point_values[index] -= INSURMOUNTABLE_COST
                        move_score_description += "{:.2f} for discarding when we could play the game out, ".format(INSURMOUNTABLE_COST)
        ...
        # Show possible actions along with their expected values
        say("----------------\n")
        for i in range(len(legal_moves)):
        ...
        return legal_moves[highest_point_move_index], highest_point_move_index, move_score_descriptions
